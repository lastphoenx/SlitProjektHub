param(
  ...
  [string]$Mode = "disable",
  [switch]$DryRun,
  # Optionale Pfade überschreiben (Standard: Workspace\app\nav_pages bzw. app\pages_labs)
  [string]$PagesDir,
  [string]$LabsDir
)
...
if (-not $PagesDir) { $PagesDir = Join-Path $Root "app\nav_pages" }
    ...
    foreach ($pattern in $LabFiles) {
      ...
      foreach ($it in $items) {
        ...
        if (-not $DryRun -and (Test-Path -LiteralPath $dst)) {
          ...
          if ($content -notmatch 'require_auth\(\)') {
            $inject = "from app.auth_gate import require_auth`r`nrequire_auth()`r`n"
            Set-Content -LiteralPath $dst -Value ($inject + $content) -NoNewline
            Write-Host "auth-gate ergänzt: $($it.Name)"
          }
        }
      }
    }
