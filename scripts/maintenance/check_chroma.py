"""ChromaDB-Index für ein Dokument prüfen (maintenance CLI)."""
...
from src.m03_db import Document, ProjectDocumentLink, get_session
from src.m07_chroma import get_chroma_client


def main() -> int:
    parser = argparse.ArgumentParser(description="ChromaDB-Chunks für ein Dokument")
    ...
    try:
        ...
        links = session.exec(
            select(ProjectDocumentLink).where(ProjectDocumentLink.document_id == doc.id)
        ).all()
