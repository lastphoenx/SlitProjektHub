...
project_key = "demo-ausschreibung-webportal"
rows = _keyword_search(query, project_key=project_key, limit=15, exclude_classification='FAQ/Fragen-Katalog')
for i, row in enumerate(rows, 1):
    print(i, row.get('filename'), row.get('classification'), row.get('match_score'))
