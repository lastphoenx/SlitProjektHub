...
from src.m09_docs import (
    IngestResult,
    extract_pdf_page_segments_with_fallback,
...
def test_extract_pdf_vision_fallback(tmp_path):
    ...
    vision_segments = [
        {"text": "Seite eins mit genug Text für die Indexierung.", "page_number": 1, "section_path": None},
        {
            "text": "Seite zwei: Vision-LLM hat den gescannten Text zuverlässig extrahiert.",
            "page_number": 2,
            "section_path": None,
        },
    ]
    ...
    with patch("src.m09_docs.extract_text_from_pdf", return_value=""):
        with patch("src.m11_vision.is_pdf_scanned", return_value=True):
            with patch("src.m11_vision.apply_ocr_to_pdf", return_value=None):
                with patch(
                    "src.m09_docs._extract_pdf_page_segments_via_vision",
                    return_value=(vision_segments, {"vision_pages_processed": 2}),
                ):
...
def test_extract_pdf_vision_segments_keep_page_numbers(tmp_path):
    pdf = tmp_path / "scan3.pdf"
    pdf.write_bytes(b"%PDF scan3")
    vision_segments = [
        {"text": "Erste Seite mit ausreichend Text für MIN_PDF_TEXT_CHARS Schwellwert.", "page_number": 1, "section_path": None},
        {"text": "Zweite Seite ebenfalls mit lesbarem Inhalt für die Volltextsuche.", "page_number": 2, "section_path": None},
    ]

    with patch("src.m09_docs._extract_pdf_pages_native", return_value=[]):
        with patch("src.m11_vision.is_pdf_scanned", return_value=True):
            with patch("src.m11_vision.apply_ocr_to_pdf", return_value=None):
                with patch(
                    "src.m09_docs._extract_pdf_page_segments_via_vision",
                    return_value=(vision_segments, {"vision_pages_processed": 2, "vision_truncated": False}),
                ):
                    segments, meta = extract_pdf_page_segments_with_fallback(pdf, use_vision_fallback=True)
...
def _run_all():
    with tempfile.TemporaryDirectory() as td:
        ...
        tests = [
            ...
            ("vision_pages", lambda: test_extract_pdf_vision_segments_keep_page_numbers(base)),
