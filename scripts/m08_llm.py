...
# Anthropic: robuste Modellauflösung (latest-Aliase -> konkrete Versionen)
ANTHROPIC_MODEL_DEFAULT = os.getenv("ANTHROPIC_MODEL", "claude-3-5-sonnet-20241022")
...
def _anthropic_try_models(system: str, user: str, *, max_tokens: int, temperature: float, preferred_alias: str = "claude-3-5-sonnet-latest") -> str | None:
    """Versucht nacheinander mehrere Anthropic-Modelle aufzurufen.
    Gibt den Text-Output zurück oder None bei Fehlschlag.
    Konfigurierbar über ANTHROPIC_MODEL und ANTHROPIC_MODEL_CANDIDATES (Komma-separiert).
    """
    try:
        import anthropic
    ...
    # Kandidatenliste: bevorzugtes Alias -> aufgelöste Version, dann ENV, dann bekannte Backups
    candidates = []
    try:
    ...
    for c in candidates_env:
        if c not in candidates:
            candidates.append(c)
    # Hart kodierte Fallbacks (stabile Reihenfolge, Dubletten vermeiden)
    hard_fallbacks = [
