...
from src.m14_auth import is_setup_required, is_super_user, session_username, validate_session_token
...
async def auth_middleware(request: Request, call_next):
    ...
    if any(path.startswith(p) for p in _AUTH_EXEMPT):
        return await call_next(request)
    # API-Endpunkte: Bearer-Token oder Cookie
...
def _session_username(request: Request) -> str:
    from src.m01_config import get_settings
    token = request.cookies.get("_auth_token", "")
    timeout = get_settings().auth_session_timeout_minutes * 60
    return session_username(token, max_age_seconds=timeout) or ""


def _require_super_user(request: Request) -> str:
    who = _session_username(request)
...
# ── CORS (Streamlit legacy + öffentliche App-Origin aus .env) ─────────────
_cors_origins = [
    "http://localhost:8501",
    "http://localhost:8502",
    "http://127.0.0.1:8501",
...
_extra = (os.getenv("PUBLIC_APP_ORIGINS") or os.getenv("CORS_ORIGINS") or "").strip()
if _extra:
...
def _doc_assignment_response(request: Request, project_key: str) -> HTMLResponse:
    """Helper: render the _documents.html partial for a given project."""
    linked = get_project_documents(project_key)
    linked_ids = {d.id for d in linked}
    all_docs = [d for d in list_documents() if d.id not in linked_ids]
    return templates.TemplateResponse("projects/_documents.html", {
        "request": request,
...
@app.get("/projects/{key}/documents", response_class=HTMLResponse)
async def project_documents_partial(request: Request, key: str):
    return _doc_assignment_response(request, key)


@app.post("/projects/{key}/documents/{doc_id}", response_class=HTMLResponse)
async def project_documents_link(request: Request, key: str, doc_id: int):
    link_document_to_project(key, doc_id)
    return _doc_assignment_response(request, key)


@app.delete("/projects/{key}/documents/{doc_id}", response_class=HTMLResponse)
async def project_documents_unlink(request: Request, key: str, doc_id: int):
    unlink_document_from_project(key, doc_id)
    return _doc_assignment_response(request, key)
...
async def roles_delete(request: Request, key: str):
...
@app.get("/documents/_table", response_class=HTMLResponse)
...
async def documents_upload(
    ...
    if not file_bytes:
        docs = list_documents()
        resp = templates.TemplateResponse("documents/_table.html", {
            "request": request,
            "docs": docs,
        })
...
@app.get("/documents/{doc_id}/confirm-delete", response_class=HTMLResponse)
async def documents_confirm_delete(request: Request, doc_id: int):
    docs = list_documents()
    doc = next((d for d in docs if d.id == doc_id), None)
    if not doc:
        raise HTTPException(status_code=404, detail="Dokument nicht gefunden")
    return templates.TemplateResponse("documents/_confirm_delete.html", {
        "request": request,
        "doc": doc,
    })
...
async def documents_delete(doc_id: int):
...
async def documents_page_image(doc_id: int, page_number: int):
    """Seitenvorschau (WebP) für RAG-Quellen in der Offertbeurteilung."""
    doc = get_document_by_id(doc_id)
    if not doc:
        raise HTTPException(status_code=404, detail="Dokument nicht gefunden")
    if page_number < 1:
        raise HTTPException(status_code=400, detail="Ungültige Seitennummer")
    path = resolve_document_page_image_path(doc_id, page_number)
    if not path:
        raise HTTPException(status_code=404, detail="Seitenvorschau nicht vorhanden")
...
async def taskgen_page(request: Request, role_key: str | None = None):
    ...
    roles_json = _json.dumps(roles_full, ensure_ascii=False).replace("<", "\\u003c")
...
async def settings_page(request: Request):
    ...
    return templates.TemplateResponse("settings/index.html", {
        ...
        "all_models_json": _all_models_json(),
        "providers": providers_available() or ["openai"],
        "provider_status": {p: have_key(p) for p in providers_available()},
        **ctx,
    })
