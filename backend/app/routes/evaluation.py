...
from src.m15_evaluation import (
    ...
    bidder_delete_summary,
    build_offer_document_assignments,
    build_offer_document_bidder_ids,
    BIDDER_DELETE_MODES,
    remove_bidder,
    restore_bidder,
...
async def evaluation_page(request: Request, project_key: str = ""):
    ...
    if project_key:
        ...
        all_project_bidders = list_bidders(project_key, include_deleted=True)
        deleted_bidders = [b for b in all_project_bidders if b.is_deleted]
        bidder_doc_subtypes = {}
        for b in all_project_bidders:
            bidder_doc_subtypes[b.id] = {}
            for doc_id in get_bidder_document_ids(b.id):
                bidder_doc_subtypes[b.id][doc_id] = get_bidder_doc_subtypes(b.id, doc_id)
        offer_doc_assignments = build_offer_document_assignments(project_key)
        offer_doc_bidder_ids = active_bidder_ids_from_assignments(offer_doc_assignments)
    else:
        deleted_bidders = []
        offer_doc_assignments = {}
        offer_doc_bidder_ids = {}
...
async def evaluation_cell(
    ...
    if children:
        ...
        ai_answered, ai_total, ai_avg = child_ai_score_summary(
            bidder_id,
        ...
        if project_key:
            ...
            parent_ctx["batch_logs"] = list_evaluation_batch_logs(
                project_key,
                bidder_id=bidder_id,
...
async def evaluation_suggest(
    ...
    tpl_ctx["rag_basis"] = enrich_rag_basis_for_display(suggestion.get("rag_basis"))
...
async def evaluation_suggest_batch_step(
    ...
    bidder_id: int = Form(...),
    criterion_id: int = Form(...),
    parent_criterion_id: int = Form(0),
    batch_run_id: str = Form(""),
    ...
    uid = get_user_id(who) if who else None
    parent_id = parent_criterion_id or 0

    with get_session() as session:
        crit = session.get(Criterion, criterion_id)
    if not crit:
        raise HTTPException(404, "Kriterium nicht gefunden")
    if not parent_id and crit.parent_id:
        parent_id = int(crit.parent_id)
    ...
    def _log_step(ok: bool, value: float | None = None, error: str | None = None) -> None:
        if not parent_id:
            return
        log_evaluation_batch_step(
            run_id=batch_run_id,
            ...
            value=value,
            error_message=error,
            llm_provider=ap or None,
            llm_model=am or None,
    ...
    if gate_err:
        _log_step(False, error=gate_err)
        return JSONResponse({
            "ok": False,
            "name": crit.name,
            "error": gate_err,
    ...
    try:
        ...
        log.exception("suggest-batch-step failed bidder=%s criterion=%s", bidder_id, criterion_id)
        _log_step(False, error=str(exc))
        return JSONResponse({
            "ok": False,
            "name": crit.name,
            "error": str(exc)[:300],
    ...
    if suggestion.get("value") is None:
        _log_step(False, error="Kein KI-Vorschlag")
        return JSONResponse({
            "ok": False,
            "name": crit.name,
    ...
    _log_step(True, value=float(suggestion["value"]))
    return JSONResponse({
        "ok": True,
        "name": crit.name,
...
@router.get("/evaluation/confirm-delete-bidder", response_class=HTMLResponse)
async def evaluation_confirm_delete_bidder(
    request: Request,
    project_key: str,
    bidder_id: int,
):
    if not can_evaluate(_username(request)):
        raise HTTPException(403, "Keine Berechtigung")
    bidder = next((b for b in list_bidders(project_key) if b.id == bidder_id), None)
    if not bidder:
        raise HTTPException(404, "Bieter nicht gefunden")
    summary = bidder_delete_summary(bidder_id)
    return templates.TemplateResponse(
        request,
        "evaluation/_confirm_delete_bidder.html",
        {
            "project_key": project_key,
            "bidder": bidder,
            "summary": summary,
            "delete_modes": BIDDER_DELETE_MODES,
        },
    )


@router.post("/evaluation/delete-bidder", response_class=HTMLResponse)
async def evaluation_delete_bidder(
    request: Request,
    project_key: str = Form(...),
    bidder_id: int = Form(...),
    mode: str = Form("deactivate"),
):
    if not can_evaluate(_username(request)):
        raise HTTPException(403, "Keine Berechtigung")
    mode = (mode or "deactivate").strip()
    if mode not in BIDDER_DELETE_MODES:
        raise HTTPException(400, "Ungültiger Löschmodus")
    try:
        result = remove_bidder(project_key, bidder_id, mode=mode)
    except ValueError as exc:
        raise HTTPException(400, str(exc))
    params = f"project_key={project_key}&bidder_removed=1&bidder_mode={result['mode']}"
    if result.get("deleted_doc_count"):
        params += f"&bidder_docs_deleted={result['deleted_doc_count']}"
    return RedirectResponse(url=f"/evaluation?{params}", status_code=303)


@router.post("/evaluation/restore-bidder", response_class=HTMLResponse)
async def evaluation_restore_bidder(
    request: Request,
    project_key: str = Form(...),
    bidder_id: int = Form(...),
):
    if not can_evaluate(_username(request)):
        raise HTTPException(403, "Keine Berechtigung")
    try:
        restore_bidder(project_key, bidder_id)
    except ValueError as exc:
        raise HTTPException(400, str(exc))
    return RedirectResponse(
        url=f"/evaluation?project_key={project_key}&bidder_restored=1",
        status_code=303,
    )
...
async def evaluation_save_config(
    ...
    rag_chunks_extraction: int = Form(36),
    ...
    save_evaluation_config(
        ...
        rag_chunks_extraction=rag_chunks_extraction,
