...
scp -r * admin@<YOUR_IP>:/home/admin/projekthub/
...
2. Verbinden zu <YOUR_IP> (User: admin)
3. Lokales Verzeichnis: `<PROJECT_ROOT>`
4. Remote: `<PROJECT_ROOT>` erstellen und alles hochladen
...
session:
  ...
  domain: <YOUR_DOMAIN>  # Deine Hauptdomain!
```
...
**Browser-Test:**
- http://<YOUR_IP>:8501 → Sollte die App zeigen! ✅
...
labels:
  - "traefik.http.routers.projekthub.rule=Host(`projekthub.<YOUR_DOMAIN>`)"
  - "traefik.http.routers.authelia.rule=Host(`auth.<YOUR_DOMAIN>`)"
```
...
Erstelle **A-Records**:
```
projekthub.<YOUR_DOMAIN>  →  DEINE-PUBLIC-IP
auth.<YOUR_DOMAIN>        →  DEINE-PUBLIC-IP
```
...
```bash
# Auf Windows-PC:
nslookup projekthub.<YOUR_DOMAIN>
...
```
Extern Port 80  → <YOUR_IP>:80   (HTTP)
Extern Port 443 → <YOUR_IP>:443  (HTTPS)
```
...
2. **Umleitung zu Authelia:**
   - Du wirst zu `https://auth.<YOUR_DOMAIN>` umgeleitet
...
- 🌐 https://projekthub.<YOUR_DOMAIN>
- 🔒 Mit 2FA-Login über https://auth.<YOUR_DOMAIN>
