...
```bash
# .env erstellen
sudo -u projekthub nano /opt/projekthub/.env
...
# Auf Server kopieren
sudo -u projekthub cp /tmp/slitproj.db /opt/projekthub/data/db/
```
...
```bash
useradd -r -s /bin/bash -d /opt/projekthub projekthub
mkdir -p /opt/projekthub
chown projekthub:projekthub /opt/projekthub
```
...
```bash
# Neu initialisieren
sudo -u projekthub /opt/projekthub/.venv/bin/python << 'EOF'
...
sudo -u projekthub cp /tmp/backup.db /opt/projekthub/data/db/slitproj.db
```
...
Environment="PATH=/opt/projekthub/.venv/bin"
EnvironmentFile=/opt/projekthub/.env
ExecStart=/opt/projekthub/.venv/bin/streamlit run app/streamlit_app.py \
    --server.port=8501 \
...
```bash
sudo -u projekthub /opt/projekthub/scripts/maintenance/01_create_backup.ps1
# Oder manuell:
cp /opt/projekthub/data/db/slitproj.db /opt/projekthub/backups/backup-$(date +%Y%m%d).db
```

### Datenbank prüfen

```bash
sudo -u projekthub /opt/projekthub/.venv/bin/python /opt/projekthub/scripts/maintenance/check_db.py
```
...
# Manuell testen
sudo -u projekthub /opt/projekthub/.venv/bin/python /opt/projekthub/backend/main.py
```
...
chown -R projekthub:projekthub /opt/projekthub

# Logs-Ordner
chmod 755 /opt/projekthub/logs
```
