...
4. Session-Cookie `_auth_token` → Middleware schützt alle anderen Routen (Timeout: `auth.session_timeout_minutes`, absolut ab Login)
5. Abgelaufene HTMX-Requests: `401` + `HX-Redirect: /auth/login` (kein POST-Redirect auf Login)
...
- Export (CSV/XLSX/HTML/DOCX/PDF/ZIP), KI-Stapel-Protokoll (`evaluation_batch_log`)

## Weitere Docs

- `docs/OFFERTBEURTEILUNG_OPS.md` — Betrieb, Reports, CLI, Troubleshooting
- `docs/AUTH.md` — Rollen, 2FA, IP-Sperre, Session-Timeout
