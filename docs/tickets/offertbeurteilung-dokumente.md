...
(Beispielkunde: Punkte = max × günstigstes / dieses Angebot). Optional `linear_minmax` in Projekt-Einstellungen.
...
**Akzeptanzkriterien:** erfüllt für Standard-Workflow (KI + Nachpflege in «Kriterien verwalten»).
...
- `EvaluationProjectConfig`: `ref_prefixes_json`, `scale_bands_json` (Defaults EK/F/R/S/T/W).
...
**Anlass:** Live-Check nach Ticket-28-Deployment (Screenshots F01-001, Bieter Alpha).
