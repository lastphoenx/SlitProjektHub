...
```yaml
auth:
  session_timeout_minutes: 60   # config/config.example.yaml
```

- Gültigkeit **ab Login-Zeit** (signiertes Cookie `_auth_token`), **keine** Verlängerung bei Aktivität.
- Lange Offertbeurteilung (KI-Vorschlag RAG, Modal offen): Session kann ablaufen, bevor «Speichern» klickt.
- HTMX ohne gültige Session: `401` + `HX-Redirect: /auth/login` (`backend/main.py`, ab Fix `f78c52d`).
- Symptom vor Fix: JSON-Fehler «username/password required» im Modal — war Redirect auf Login mit falschem POST-Body.

Details + Workaround: `docs/OFFERTBEURTEILUNG_OPS.md` §2.
