...
# Preis-Punkte: reciprocal = max × (günstigstes / Angebot) — typische Ausschreibungsvorgabe
...
class EvaluationBatchLog(SQLModel, table=True):
    """Protokoll je Schritt der KI-Stapelverarbeitung (Parent-Modal)."""
    __tablename__ = "evaluation_batch_log"
    id: Optional[int] = Field(default=None, primary_key=True)
    run_id: str = Field(sa_column=Column(String(36), nullable=False, index=True))
    project_key: str = Field(sa_column=Column(String(80), nullable=False, index=True))
    bidder_id: int = Field(foreign_key="bidder.id", index=True)
    parent_criterion_id: int = Field(foreign_key="criterion.id", index=True)
    criterion_id: int = Field(foreign_key="criterion.id", index=True)
    step_index: int = Field(default=0, sa_column=Column(Integer, nullable=False, default=0))
    ok: bool = Field(default=False, sa_column=Column(Boolean, nullable=False, default=False))
    ...
    triggered_by_user_id: Optional[int] = Field(default=None, foreign_key="app_user.id", index=True)
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
...
BIDDER_DELETE_MODES = ("deactivate", "unlink", "delete_exclusive_docs")


def restore_bidder(project_key: str, bidder_id: int) -> Bidder:
    """Gelöschten Bieter reaktivieren — Dokument-Links bleiben erhalten."""
    with get_session() as session:
        bidder = session.get(Bidder, bidder_id)
        if not bidder or bidder.project_key != project_key:
            raise ValueError("Bieter nicht gefunden")
        if not bidder.is_deleted:
            raise ValueError("Bieter ist nicht gelöscht")
        bidder.is_deleted = False
        session.add(bidder)
        session.commit()
        session.refresh(bidder)
        return bidder
...
def _exclusive_document_ids_for_bidder(bidder_id: int) -> list[int]:
    """Dokumente, die nur mit diesem Bieter verknüpft sind."""
    doc_ids = get_bidder_document_ids(bidder_id)
    if not doc_ids:
        return []
    exclusive: list[int] = []
    with get_session() as session:
        for doc_id in doc_ids:
            links = session.exec(
                select(BidderDocumentLink).where(BidderDocumentLink.document_id == doc_id)
            ).all()
            if len(links) == 1 and int(links[0].bidder_id) == int(bidder_id):
                exclusive.append(int(doc_id))
    return exclusive


def bidder_delete_summary(bidder_id: int) -> dict[str, Any]:
    """Zahlen für den Löschen-Dialog."""
    linked = get_bidder_document_ids(bidder_id)
    exclusive = _exclusive_document_ids_for_bidder(bidder_id)
    return {
        "linked_doc_count": len(linked),
        "exclusive_doc_count": len(exclusive),
    }
...
def remove_bidder(
    ...
    """
    Bieter entfernen (soft-delete).
    mode: deactivate | unlink | delete_exclusive_docs
    """
    mode = (mode or "deactivate").strip()
    if mode not in BIDDER_DELETE_MODES:
    ...
    with get_session() as session:
        ...
        if not bidder or bidder.project_key != project_key:
            raise ValueError("Bieter nicht gefunden")
        if bidder.is_deleted:
            raise ValueError("Bieter ist bereits entfernt")

    doc_ids = list(get_bidder_document_ids(bidder_id))
    deleted_docs: list[int] = []

    if mode == "unlink":
        for doc_id in doc_ids:
            unlink_document_from_bidder(bidder_id, doc_id)
    elif mode == "delete_exclusive_docs":
        from .m09_docs import delete_document

        for doc_id in _exclusive_document_ids_for_bidder(bidder_id):
...
def soft_delete_bidder(bidder_id: int) -> None:
    """Legacy: nur deaktivieren, Links bleiben. Bereits entfernt → stilles No-Op."""
    with get_session() as session:
        bidder = session.get(Bidder, bidder_id)
        if not bidder or bidder.is_deleted:
            return
        project_key = bidder.project_key
    remove_bidder(project_key, bidder_id, mode="deactivate")
...
def child_ai_score_summary(
    bidder_id: int,
    children: list[Criterion],
    scores_by_cell: dict[tuple[int, int], list[Score]],
) -> tuple[int, int, Optional[float]]:
    """
    KI-Vorschläge (source=ai) auf Unterfragen — für Anzeige, nicht für official_score.
    Gibt (ki_beantwortet, gesamt, ki_mittel) zurück.
    """
    if not children:
        return 0, 0, None
    ai_vals: list[float] = []
    for ch in children:
        ...
        if ai_row is not None and ai_row.value is not None:
            ai_vals.append(float(ai_row.value))
    total = len(children)
    if not ai_vals:
        return 0, total, None
    return len(ai_vals), total, round(sum(ai_vals) / len(ai_vals), 2)
...
def log_evaluation_batch_step(
    *,
    run_id: str,
    project_key: str,
    bidder_id: int,
    parent_criterion_id: int,
    criterion_id: int,
    step_index: int,
    ok: bool,
    ...
    """Einzelnen Schritt der KI-Stapelverarbeitung persistieren."""
    msg = (error_message or "").strip() or None
    if msg and len(msg) > 500:
        msg = msg[:497] + "..."
    row = EvaluationBatchLog(
...
def list_evaluation_batch_logs(
    project_key: str,
    *,
    bidder_id: int | None = None,
    parent_criterion_id: int | None = None,
    run_id: str | None = None,
    since_days: int | None = None,
    limit: int = 100,
) -> list[dict[str, Any]]:
    """Batch-Protokoll mit Bieter- und Kriteriumsnamen (UI/CLI)."""
    from .m14_auth import get_username_by_id

    q = (
        select(EvaluationBatchLog)
        .where(EvaluationBatchLog.project_key == project_key)
        .order_by(EvaluationBatchLog.created_at.desc(), EvaluationBatchLog.id.desc())
    )
    if bidder_id is not None:
        q = q.where(EvaluationBatchLog.bidder_id == bidder_id)
    if parent_criterion_id is not None:
        q = q.where(EvaluationBatchLog.parent_criterion_id == parent_criterion_id)
    if run_id:
        q = q.where(EvaluationBatchLog.run_id == run_id.strip())
    if since_days is not None and since_days > 0:
        cutoff = datetime.now(timezone.utc).replace(microsecond=0) - timedelta(days=since_days)
        q = q.where(EvaluationBatchLog.created_at >= cutoff)

    with get_session() as session:
        rows = session.exec(q.limit(max(1, min(limit, 500)))).all()
        if not rows:
            return []

        bidder_ids = {int(r.bidder_id) for r in rows}
        crit_ids = {int(r.criterion_id) for r in rows} | {int(r.parent_criterion_id) for r in rows}
        bidders = {
            int(b.id): b
            for b in session.exec(select(Bidder).where(Bidder.id.in_(bidder_ids))).all()
            if b.id is not None
        }
        criteria = {
            int(c.id): c
            for c in session.exec(select(Criterion).where(Criterion.id.in_(crit_ids))).all()
            if c.id is not None
        }

        out: list[dict[str, Any]] = []
        for row in rows:
            bidder = bidders.get(int(row.bidder_id))
            crit = criteria.get(int(row.criterion_id))
            parent = criteria.get(int(row.parent_criterion_id))
            user_name = None
            if row.triggered_by_user_id:
                user_name = get_username_by_id(int(row.triggered_by_user_id))
            out.append({
                "id": row.id,
                "run_id": row.run_id,
                "project_key": row.project_key,
                "bidder_id": row.bidder_id,
                "bidder_name": bidder.name if bidder else f"#{row.bidder_id}",
                "bidder_deleted": bool(bidder.is_deleted) if bidder else False,
                "parent_criterion_id": row.parent_criterion_id,
                "parent_referenz": parent.referenz if parent else None,
                "parent_name": parent.name if parent else f"#{row.parent_criterion_id}",
                "criterion_id": row.criterion_id,
                "criterion_referenz": crit.referenz if crit else None,
                "criterion_name": crit.name if crit else f"#{row.criterion_id}",
...
def build_offer_document_bidder_ids(
    bidder_docs: dict[int, list[int]],
) -> dict[int, list[int]]:
    """document_id → aktive verknüpfte bidder_ids (sortiert)."""
    out: dict[int, list[int]] = {}
    for bidder_id, doc_ids in bidder_docs.items():
        for doc_id in doc_ids:
            out.setdefault(int(doc_id), []).append(int(bidder_id))
    for doc_id in out:
        out[doc_id] = sorted(set(out[doc_id]))
    return out


def build_offer_document_assignments(project_key: str) -> dict[int, list[dict[str, Any]]]:
    """
    document_id → Zuordnungen inkl. gelöschter Bieter
    [{bidder_id, name, is_deleted}, ...]
    """
    with get_session() as session:
        bidders = session.exec(
            select(Bidder).where(Bidder.project_key == project_key)
        ).all()
        bidder_map = {int(b.id): b for b in bidders if b.id is not None}
        if not bidder_map:
            return {}
        links = session.exec(
            select(BidderDocumentLink).where(
                BidderDocumentLink.bidder_id.in_(bidder_map.keys())
            )
        ).all()

    out: dict[int, list[dict[str, Any]]] = {}
    for link in links:
        bidder = bidder_map.get(int(link.bidder_id))
        if not bidder:
            continue
        out.setdefault(int(link.document_id), []).append(
            {
                "bidder_id": int(bidder.id),
                "name": bidder.name,
                "is_deleted": bool(bidder.is_deleted),
            }
        )
    for doc_id in out:
        out[doc_id].sort(key=lambda row: (row["is_deleted"], row["name"].lower()))
    return out


def active_bidder_ids_from_assignments(
    assignments: dict[int, list[dict[str, Any]]],
) -> dict[int, list[int]]:
    """document_id → nur aktive bidder_ids (für Matrix-Fokus-Filter)."""
    out: dict[int, list[int]] = {}
    for doc_id, rows in assignments.items():
        active = sorted(
            {
                int(row["bidder_id"])
                for row in rows
...
def normalize_ref_prefixes(raw: Any) -> list[str]:
    """Projekt-Referenz-Präfixe (Ticket 26), Default EK/F/R/S/T/W."""
...
def _dedupe_rag_docs(docs: list[dict]) -> list[dict]:
    ...
    for d in docs:
        ...
        if new_pri < old_pri:
            ...
            merged.update(d)
            by_id[key] = merged
    return [by_id[key] for key in order] + extras
...
def parse_rag_basis_json(raw: str | None) -> dict[str, Any] | None:
    ...
    if not isinstance(data, dict):
        return None
    return enrich_rag_basis_for_display(data)


def _page_number_from_chunk_text(chunk_text: str) -> int | None:
    """Fallback: Seite aus Chunk-Prefix [… | S. 12] lesen."""
    head = (chunk_text or "").split("\n", 1)[0]
    m = re.search(r"S\.\s*(\d+)", head)
    if not m:
        return None
    try:
        return int(m.group(1))
...
def _collect_rag_basis_chunk_ids(
    bases: Iterable[dict[str, Any] | None],
) -> list[int]:
    chunk_ids: list[int] = []
    for basis in bases:
        if not basis:
        ...
        for key in ("tender", "offer"):
            for item in basis.get(key) or []:
                if not isinstance(item, dict):
                    continue
                try:
                    chunk_ids.append(int(item.get("chunk_id")))
                except (TypeError, ValueError):
                    continue
    return sorted(set(chunk_ids))


def _load_rag_enrich_maps(
    chunk_ids: list[int],
    ...
    with get_session() as session:
        ...
        chunk_map = {int(ch.id): ch for ch in rows if ch.id is not None}
        doc_ids = {int(ch.document_id) for ch in rows if ch.document_id}
        doc_map: dict[int, Document] = {}
        if doc_ids:
            docs = session.exec(
                select(Document).where(Document.id.in_(doc_ids))
            ).all()
            doc_map = {int(d.id): d for d in docs if d.id is not None}
    return chunk_map, doc_map


def _collect_rag_page_pairs(
    bases: Iterable[dict[str, Any] | None],
) -> set[tuple[int, int]]:
    ...
    for basis in bases:
        ...
        for key in ("tender", "offer"):
            for item in basis.get(key) or []:
                ...
                if doc_id is None or pn is None:
                    continue
                try:
                    pairs.add((int(doc_id), int(pn)))
                except (TypeError, ValueError):
                    continue
    return pairs


def _load_rag_page_image_paths(pairs: set[tuple[int, int]]) -> dict[tuple[int, int], str]:
    """Ein DB-Roundtrip für alle Seitenvorschau-Pfade eines Export-Laufs."""
    if not pairs:
        return {}
    from .m03_db import DocumentPage
    from .m09_docs import PAGES_DIR
    ...
    for row in rows:
        if row.document_id is None or row.page_number is None:
            continue
    ...
    for doc_id, pn in pairs:
        if (doc_id, pn) in out:
            continue
        fallback = PAGES_DIR / str(doc_id) / f"{pn}.webp"
        if fallback.is_file():
            out[(doc_id, pn)] = str(fallback)
    return out


def enrich_rag_basis_for_display(
    basis: dict[str, Any] | None,
    *,
    chunk_map: dict[int, Any] | None = None,
    doc_map: dict[int, Any] | None = None,
) -> dict[str, Any] | None:
    """
    Ergänzt gespeicherte rag_basis-Einträge um document_id/page_number aus document_chunk
    (für Thumbnails nach Page-Image-Backfill, ohne neuen KI-Lauf).
    """
    if not basis:
        return None

    if chunk_map is None or doc_map is None:
        chunk_ids = _collect_rag_basis_chunk_ids([basis])
        loaded_chunk_map, loaded_doc_map = _load_rag_enrich_maps(chunk_ids)
        chunk_map = chunk_map if chunk_map is not None else loaded_chunk_map
        doc_map = doc_map if doc_map is not None else loaded_doc_map
    ...
    for key in ("tender", "offer"):
        ...
        for item in basis.get(key) or []:
            ...
            row = dict(item)
            try:
                cid_int = int(row.get("chunk_id"))
            except (TypeError, ValueError):
                cid_int = None
            ch = chunk_map.get(cid_int) if cid_int is not None else None
            if ch:
                if not row.get("document_id"):
                    row["document_id"] = ch.document_id
                if not row.get("page_number") and ch.page_number:
                    row["page_number"] = ch.page_number
                if not row.get("section_path") and ch.section_path:
                    row["section_path"] = ch.section_path
                if not row.get("page_number"):
                    pn = _page_number_from_chunk_text(ch.chunk_text or "")
                    if pn:
                        row["page_number"] = pn
            ...
            if doc_id:
                ...
                if doc:
                    ...
                    if fn.endswith(".pdf"):
                        ...
                        row["doc_kind"] = "other"
                    row["document_preview_url"] = f"/documents/{doc.id}/preview"
            out_items.append(row)
        enriched[key] = out_items
    return enriched
...
def prepare_rag_basis_for_export(
    basis: dict[str, Any] | None,
    *,
    chunk_map: dict[int, Any] | None = None,
    doc_map: dict[int, Any] | None = None,
    page_image_paths: dict[tuple[int, int], str] | None = None,
) -> dict[str, Any] | None:
    """Enrich + eingebettete Seitenvorschau-Pfade/Data-URIs für PDF/DOCX/Excel."""
    enriched = enrich_rag_basis_for_display(
        basis, chunk_map=chunk_map, doc_map=doc_map
    )
    if not enriched:
        return None
    if page_image_paths is None:
        page_image_paths = _load_rag_page_image_paths(_collect_rag_page_pairs([enriched]))
    return _attach_page_images_to_rag_basis(enriched, page_image_paths)


def prepare_rag_bases_for_export_batch(
    raw_bases: list[dict[str, Any] | None],
) -> list[dict[str, Any] | None]:
    """Batch-Vorbereitung für Export — eine DB-Session für Chunks/Docs/Bilder."""
    parsed = [b for b in raw_bases if b]
    if not parsed:
        return [None for _ in raw_bases]
    chunk_map, doc_map = _load_rag_enrich_maps(_collect_rag_basis_chunk_ids(parsed))
    enriched_list = [
        enrich_rag_basis_for_display(b, chunk_map=chunk_map, doc_map=doc_map) for b in parsed
    ]
    page_pairs = _collect_rag_page_pairs([e for e in enriched_list if e])
    page_image_paths = _load_rag_page_image_paths(page_pairs)
    prepared_parsed = [
        _attach_page_images_to_rag_basis(e, page_image_paths) if e else None
    ...
    for basis in raw_bases:
        if not basis:
            ...
            out.append(prepared_parsed[parsed_idx])
            parsed_idx += 1
    return out


def format_rag_basis_export_text(rag_basis: dict[str, Any] | None) -> str:
    """Alle gespeicherten Quellen als Fliesstext für CSV/Excel."""
    if not rag_basis:
        return ""
    lines: list[str] = []
    for section_key, section_title in (("tender", "Vorgaben"), ("offer", "Angebot")):
        items = rag_basis.get(section_key) or []
        if not items:
            continue
        ...
        for item in items:
            if not isinstance(item, dict):
                continue
            head = item.get("filename") or "—"
            if item.get("page_number"):
                head += f" · S. {item['page_number']}"
            ...
            if score is not None:
                try:
                    sf = float(score)
                    head += f" · {sf:.0%}" if sf <= 1.0 else f" · {sf:.2f}"
                except (TypeError, ValueError):
                    pass
            preview = (item.get("preview") or "").strip()
            lines.append(f"  • {head}: {preview}" if preview else f"  • {head}")
    return "\n".join(lines)
...
def collect_rag_basis_page_image_paths(rag_basis: dict[str, Any] | None) -> list[str]:
    ...
    for key in ("tender", "offer"):
        for item in rag_basis.get(key) or []:
            if not isinstance(item, dict):
                continue
            img_path = item.get("page_image_path")
            doc_id = item.get("document_id")
            pn = item.get("page_number")
            if not img_path or doc_id is None or pn is None:
                continue
            try:
                dedupe_key = (int(doc_id), int(pn))
            except (TypeError, ValueError):
                continue
...
def _xlsx_embed_stacked_thumbnails(ws, row_idx: int, col_idx: int, image_paths: list[str], *, thumb_width: int = 130) -> int:
    """Thumbnails untereinander in einer Zelle; gibt Pixel-Höhe zurück."""
    if not image_paths:
        return 0
    ...
    for path in image_paths:
        ...
        size = XDRPositiveSize2D(pixels_to_EMU(thumb_width), pixels_to_EMU(height))
...
def build_evaluation_export_context(
    ...
    *,
    ...
    export_criteria = top_criteria + child_criteria
    raw_rag_bases: list[dict[str, Any] | None] = []
    for crit in export_criteria:
        cell = scores_by_cell.get((bidder_id, crit.id), [])
        sc = _score_for_source(cell, source_key)
        raw_rag_bases.append(
            _parse_rag_basis_json(sc.rag_basis_json if sc else None)
        )
    prepared_rag_bases = prepare_rag_bases_for_export_batch(raw_rag_bases)

    top_rows: list[EvaluationExportRow] = []
    for idx, crit in enumerate(top_criteria):
        cell = scores_by_cell.get((bidder_id, crit.id), [])
        top_rows.append(
            _export_row_from_score(
                crit,
                _score_for_source(cell, source_key),
                rag_basis=prepared_rag_bases[idx],
            )
        )

    child_rows: list[EvaluationExportRow] = []
    top_count = len(top_criteria)
    for offset, crit in enumerate(child_criteria):
        cell = scores_by_cell.get((bidder_id, crit.id), [])
        child_rows.append(
            _export_row_from_score(
                crit,
                ...
                rag_basis=prepared_rag_bases[top_count + offset],
            )
        )
...
def _context_to_full_tabular_sheets(
    ...
    main_headers = [
        ...
        "Wert", "Begründung", "Quellenreferenz", "Nachweise", "Seitenvorschau",
    ]
    ...
    for row in ctx.top_rows:
        main_rows.append(
            ...
            + [
                ...
                format_rag_basis_export_text(row.rag_basis)
                if ctx.source_key == "ai"
                else "",
                "",
            ]
        )
    ...
    detail_headers = [
        ...
        "Art", "Skala", "Wert", "Begründung", "Quellenreferenz", "Nachweise", "Seitenvorschau",
    ]
...
def _write_xlsx_sheet_with_thumbnails(
    ws,
    headers: list[str],
    rows: list[list[Any]],
    export_rows: list[EvaluationExportRow],
    *,
    source_key: str,
) -> None:
    ...
    ws.append(headers)
    thumb_col = headers.index("Seitenvorschau") + 1 if "Seitenvorschau" in headers else None
    wrap = Alignment(wrap_text=True, vertical="top")
    for row_idx, (values, export_row) in enumerate(zip(rows, export_rows), start=2):
        ws.append(values)
        for col_idx in range(1, len(headers) + 1):
            ws.cell(row=row_idx, column=col_idx).alignment = wrap
        if thumb_col and source_key == "ai":
            paths = collect_rag_basis_page_image_paths(export_row.rag_basis)
            if paths:
                height_px = _xlsx_embed_stacked_thumbnails(ws, row_idx, thumb_col, paths)
                ws.row_dimensions[row_idx].height = max(15, min(height_px * 0.75, 409))
    if thumb_col:
        ws.column_dimensions[get_column_letter(thumb_col)].width = 22
    for col_idx, header in enumerate(headers, start=1):
        if header == "Nachweise":
            ws.column_dimensions[get_column_letter(col_idx)].width = 48
        elif header in ("Begründung", "Quellenreferenz"):
...
def build_filtered_xlsx_bytes(ctx: EvaluationExportContext) -> bytes:
    ...
    headers, rows = sheets["Bewertungen"]
    _write_xlsx_sheet_with_thumbnails(
        ws, headers, rows, ctx.top_rows, source_key=ctx.source_key
    )
...
def _docx_append_score_evidence(doc, row: EvaluationExportRow, ctx: EvaluationExportContext) -> None:
    """Quellenreferenz, rag_basis-Text und Seitenvorschauen nach einer Tabellenzeile."""
    from docx.shared import Inches

    embedded_pages: set[tuple[int, int]] = set()

    if row.source_chunk_ref:
        doc.add_paragraph(row.source_chunk_ref, style="Intense Quote")
    if not row.rag_basis or ctx.source_key != "ai":
        return
    for section_key, section_title in (("tender", "Vorgaben"), ("offer", "Angebot")):
        ...
        for item in items:
            ...
            if score is not None:
                try:
                    sf = float(score)
                    line += f" · {sf:.0%}" if sf <= 1.0 else f" · {sf:.2f}"
                except (TypeError, ValueError):
                    pass
            preview = (item.get("preview") or "").strip()
            doc.add_paragraph(f"{line}: {preview}" if preview else line, style="List Bullet 2")
            img_path = item.get("page_image_path")
            doc_id = item.get("document_id")
            pn = item.get("page_number")
            if not img_path or doc_id is None or pn is None:
    ...
    def _add_criteria_table(title: str, rows: list[EvaluationExportRow], *, child: bool = False) -> None:
        ...
        if child:
            ...
            for row in rows:
                ...
                _docx_append_score_evidence(doc, row, ctx)
        else:
