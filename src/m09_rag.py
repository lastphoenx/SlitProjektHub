...
def retrieve_relevant_chunks_hybrid(query: str, project_key: str | None = None, limit: int = 5, threshold: float = 0.5) -> dict:
    ...
    if semantic_results.get("documents") and keyword_docs:
        semantic_ids = {d["document_id"] for d in semantic_results["documents"]}
        
        for keyword_doc in keyword_docs:
            if keyword_doc["document_id"] not in semantic_ids:
                semantic_results["documents"].append(keyword_doc)
        
        # Re-sort: Semantic-Treffer oben
        semantic_results["documents"] = sorted(
            semantic_results["documents"],
            key=lambda x: x.get("similarity", x.get("match_score", 0)),
            reverse=True
        )[:limit]
    elif keyword_docs and not semantic_results.get("documents"):
        semantic_results["documents"] = keyword_docs
...
def retrieve_relevant_chunks(query: str, project_key: str | None = None, limit: int = 5, threshold: float = 0.5) -> dict:
    ...
    with get_session() as ses:
        ...
        results["documents"] = sorted(doc_scores, key=lambda x: x["similarity"], reverse=True)[:limit]
