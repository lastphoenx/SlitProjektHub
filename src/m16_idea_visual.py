...
# Grobe Heuristik «Herr/Frau Name» — entfernt typische Anrede+Name-Kombinationen
...
def _sanitize_for_cloud_text(text: str) -> str:
    """Entfernt offensichtliche PII vor Cloud-Prompt (zusätzlich zur LLM-Regel)."""
    if not text:
        return ""
...
def _build_illustration_user_prompt(idea: ProjectIdea) -> str:
    """Nur ai_* und Fachabteilung — niemals idea_text."""
    parts: list[str] = []
    if idea.ai_project_name:
        parts.append(f"Projektname (generisch): {_sanitize_for_cloud_text(idea.ai_project_name)}")
    if idea.fachabteilung:
        parts.append(f"Fachbereich/Organisation: {_sanitize_for_cloud_text(idea.fachabteilung)}")
    if idea.ai_summary:
        parts.append(f"Sachliche Kurzfassung: {_sanitize_for_cloud_text(idea.ai_summary)}")
    if idea.ai_recommendation:
        parts.append(f"Empfehlung (abstrakt): {_sanitize_for_cloud_text(idea.ai_recommendation)}")
    if not parts:
...
def build_dsgvo_illustration_prompt(
    ...
    """
    Lokaler LLM-Schritt: abstrakter DALL-E-Prompt ohne sensible Daten.
    Fallback ohne LLM wenn nur Minimaldaten.
    """
    user = _build_illustration_user_prompt(idea)
    ...
    prompt = _sanitize_for_cloud_text((raw or "").strip().strip('"').strip("'"))
    if len(prompt) < 20:
        title = _sanitize_for_cloud_text(idea.ai_project_name or idea.title or "Public sector project")
        dept = _sanitize_for_cloud_text(idea.fachabteilung or "")
        prompt = (
...
def generate_openai_illustration(prompt: str, image_model: str = DEFAULT_OPENAI_IMAGE_MODEL) -> Optional[bytes]:
    """OpenAI Images API — nur mit bereits bereinigtem Prompt."""
...
def _add_title_slide(prs: Presentation, idea: ProjectIdea) -> None:
    slide = prs.slides.add_slide(prs.slide_layouts[6])  # blank
    title = idea.ai_project_name or idea.title or f"Projektidee #{idea.id}"
    left, top, width, height = Inches(0.6), Inches(1.2), Inches(9), Inches(1.2)
    ...
    sub_parts = []
    if idea.fachabteilung:
        sub_parts.append(idea.fachabteilung)
    sub_parts.append("Projektportfolio — Ersteinschätzung")
    sub_parts.append(idea.created_at.strftime("%d.%m.%Y"))
    sub = slide.shapes.add_textbox(Inches(0.6), Inches(2.4), Inches(9), Inches(1))
    stf = sub.text_frame
    sp = stf.paragraphs[0]
    sp.text = " · ".join(sub_parts)
    sp.font.size = Pt(14)
...
def _build_deck_bytes(idea: ProjectIdea) -> bytes:
    prs = Presentation()
    prs.slide_width = Inches(10)
    prs.slide_height = Inches(7.5)

    _add_title_slide(prs, idea)

    if idea.ai_summary:
        sentences = [s.strip() for s in re.split(r"(?<=[.!?])\s+", idea.ai_summary) if s.strip()]
        _add_bullet_slide(prs, "Zusammenfassung", sentences[:6])

    res_lines = []
    if idea.ai_internal_pt is not None:
        res_lines.append(f"Intern: {idea.ai_internal_pt:.0f} Personentage")
        if idea.ai_internal_pt_reasoning:
            res_lines.append(idea.ai_internal_pt_reasoning)
    if idea.ai_external_cost is not None:
        res_lines.append(f"Extern: CHF {idea.ai_external_cost:,.0f}".replace(",", "'"))
        if idea.ai_external_cost_reasoning:
            res_lines.append(idea.ai_external_cost_reasoning)
    if res_lines:
        _add_bullet_slide(prs, "Ressourcen & Kosten (KI-Schätzung)", res_lines)
    ...
    if challenges:
        ...
        _add_bullet_slide(prs, "Herausforderungen", ch_lines)
    ...
    if phases:
        ...
        _add_bullet_slide(prs, "Grobe Phasenplanung", ph_lines)

    if idea.ai_recommendation:
        _add_bullet_slide(prs, "Empfehlung", [idea.ai_recommendation])

    foot = prs.slides.add_slide(prs.slide_layouts[6])
    fb = foot.shapes.add_textbox(Inches(0.6), Inches(3), Inches(9), Inches(1))
...
def _build_deck_preview_png(idea: ProjectIdea) -> bytes:
    w, h = 1280, 720
    img = Image.new("RGB", (w, h), color=(30, 58, 95))
    draw = ImageDraw.Draw(img)
    title = (idea.ai_project_name or idea.title or f"Idee #{idea.id}")[:80]
    try:
    ...
    y = 180
    if idea.ai_summary:
        summary = idea.ai_summary[:280] + ("…" if len(idea.ai_summary) > 280 else "")
        draw.text((60, y), summary, fill=(200, 210, 220), font=font_s)
        y += 120
    if idea.ai_internal_pt is not None:
        draw.text((60, y), f"Intern: {idea.ai_internal_pt:.0f} PT", fill=(180, 200, 230), font=font_s)
        y += 36
    if idea.ai_external_cost is not None:
        draw.text((60, y), f"Extern: CHF {idea.ai_external_cost:,.0f}".replace(",", "'"), fill=(180, 200, 230), font=font_s)
...
def generate_portfolio_deck(idea_id: int) -> Optional[ProjectIdea]:
    ...
    deck_path = idea_decks_dir() / deck_name
    deck_path.write_bytes(_build_deck_bytes(idea))
    (idea_images_dir() / preview_name).write_bytes(_build_deck_preview_png(idea))
...
def generate_cloud_illustration(
    ...
    safe_prompt = build_dsgvo_illustration_prompt(idea, llm_provider, llm_model)
    if not safe_prompt:
        return None
