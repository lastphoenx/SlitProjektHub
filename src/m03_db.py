...
class DocumentPage(SQLModel, table=True):
    """Rasterisierte PDF-Seite für Quellen-Vorschau in der Offertbeurteilung."""
...
def migrate_db() -> None:
    ...
    with engine.begin() as conn:
        ...
        if not _column_exists("document_page", "document_id"):
            conn.execute(text("""
            ...
            """))
            conn.execute(text(
                "CREATE INDEX IF NOT EXISTS idx_document_page_doc ON document_page(document_id);"
            ))
        ...
        # Auth rework (app_user / app_role) — create_all ergänzt keine Spalten
        if _column_exists("app_user", "id"):
            if not _column_exists("app_user", "password_hash"):
                conn.execute(text("ALTER TABLE app_user ADD COLUMN password_hash VARCHAR;"))
            if not _column_exists("app_user", "totp_secret"):
                conn.execute(text("ALTER TABLE app_user ADD COLUMN totp_secret VARCHAR;"))
            if not _column_exists("app_user", "totp_enabled"):
                conn.execute(text("ALTER TABLE app_user ADD COLUMN totp_enabled BOOLEAN DEFAULT 0;"))
            if not _column_exists("app_user", "totp_required"):
                conn.execute(text("ALTER TABLE app_user ADD COLUMN totp_required BOOLEAN;"))
            if not _column_exists("app_user", "app_role_key"):
                conn.execute(text("ALTER TABLE app_user ADD COLUMN app_role_key VARCHAR(64);"))
            if not _column_exists("app_user", "session_epoch"):
                conn.execute(
                    text("ALTER TABLE app_user ADD COLUMN session_epoch INTEGER NOT NULL DEFAULT 1;")
                )
            if not _column_exists("app_user", "is_active"):
                conn.execute(text("ALTER TABLE app_user ADD COLUMN is_active BOOLEAN DEFAULT 1;"))
            if not _column_exists("app_user", "created_at"):
                conn.execute(text("ALTER TABLE app_user ADD COLUMN created_at DATETIME;"))
        if _column_exists("app_role", "key") and not _column_exists("app_role", "totp_required"):
            conn.execute(text("ALTER TABLE app_role ADD COLUMN totp_required BOOLEAN;"))
...
def init_db() -> None:
    ...
    from src.m14_auth import (  # noqa: F401
        AppRole,
        AppUser,
        IpBlocklist,
        LoginAttempt,
    ...
    for attempt in range(1, 6):
        try:
            ...
            if "locked" in str(exc).lower() and attempt < 5:
                log.warning("init_db: DB locked, retry %s/5", attempt)
                time.sleep(1.0)
                continue
            raise
